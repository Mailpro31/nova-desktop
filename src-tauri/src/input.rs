...
///
/// Mirrors [`send_paste_ctrl_v`] exactly — same key-code strategy (layout
/// independent), same press/click/release sequence, same inter-key delay — so
/// the two behave identically across keyboard layouts.
///
/// Used only by the Nova Commands selection capture (`nova_commands.rs`); the
/// dictation pipeline never sends a copy keystroke.
